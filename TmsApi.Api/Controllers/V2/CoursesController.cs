...
public class CoursesController(IMediator mediator) : ControllerBase
{
    ...
    [HttpPut("{code}")]
    public async Task<IActionResult> Update(string code, UpdateCourseRequest request, CancellationToken ct)
    {
        var command = new UpdateCourseCommand(code, request.Title, request.MaxCapacity);
    ...
    {
        var deleted = await mediator.Send(new DeleteCourseCommand(code), ct);
