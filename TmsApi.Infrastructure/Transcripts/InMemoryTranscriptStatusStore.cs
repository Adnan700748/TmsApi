...
{
    private readonly ConcurrentDictionary<string, TranscriptStatus>
        _byReportId = new();

    private readonly ConcurrentDictionary<string, string>
        _idempotencyToReportId = new();

    public Task<TranscriptStatus> CreateAsync(string reportId, int studentId, CancellationToken ct)
    {
        var status = new TranscriptStatus(reportId, studentId, TranscriptState.Queued, RequestedAt: DateTimeOffset.UtcNow);
    ...
    {
        return Task.FromResult(
            _idempotencyToReportId.TryGetValue(key, out var id)
                ? id
                : null);
    }
