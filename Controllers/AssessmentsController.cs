    ...
    LinkGenerator linkGenerator) : ControllerBase
{
    [HttpGet]
    [ProducesResponseType(
        typeof(PagedResponse<AssessmentResponseDto>),
        StatusCodes.Status200OK)]
    [EndpointSummary("List assessments with pagination")]
    [EndpointDescription(
        "Returns a paginated, optionally filtered list of TMS assessments. PageSize is capped at 50.")]
    public async Task<IActionResult> GetAssessments(
        [FromQuery] PagedRequest request,
        CancellationToken ct)
    {
        var result =
            await assessmentService.GetAssessmentsAsync(request, ct);
    ...
    [EndpointDescription(
        "Returns assessment details with HATEOAS links. Returns 404 if the assessment does not exist.")]
    public async Task<IActionResult> GetAssessmentById(
        int id,
        CancellationToken ct)
    {
        var assessment =
            await assessmentService.GetByIdAsync(id, ct);
    ...
    {
        var result =
            await assessmentService.CreateAsync(request, ct);
